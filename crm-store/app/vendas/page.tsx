...
export default function VendasPage() {
  ...
  const [paymentMethod, setPaymentMethod] = useState<
    "cash" | "card" | "pix" | "transfer" | "other"
  >("cash");
  const [formData, setFormData] = useState<SaleFormData>({
    customer: null,
    first_due_date: new Date().toISOString().split("T")[0],
    installments_count: 1,
    items: [{ product: "", quantity: 1, sale_price: "" }],
  });
  ...
  async function fetchData() {
    try {
      const [salesRes, installmentsRes, clientsRes, productsRes] =
        await Promise.all([
          saleService.getAll(),
  ...
  const handleAddItem = () => {
    setFormData({
      ...formData,
      items: [...formData.items, { product: "", quantity: 1, sale_price: "" }],
    });
  };
      ...
      if (field === "product") {
        ...
        if (product) {
          ...
          const priceDisplay = formatCurrencyInput(
            ((Number(pp) || 0) * 1.3).toFixed(2).replace(".", ""),
          );
          newItems[index] = {
            ...newItems[index],
            sale_price: ((Number(pp) || 0) * 1.3).toFixed(2),
  ...
  const handleOpenNewSale = () => {
    setFormData({
      ...
      installments_count: 1,
      items: [
        { product: "", quantity: 1, sale_price: "", sale_price_display: "" },
      ],
    });
    setSaleType("vista");
    setIsModalOpen(true);
  ...
  const handleSave = async () => {
    ...
    try {
      const finalData = {
        ...formData,
        installments_count:
          saleType === "vista" ? 1 : formData.installments_count,
        is_paid: saleType === "vista",
        payment_method: saleType === "vista" ? paymentMethod : undefined,
      };
      const validItems = finalData.items.filter(
        ...
        <div className="flex flex-col gap-4">
          <Select
            ...
            onChange={(e) =>
              setFormData({
                ...formData,
                customer: e.target.value ? String(e.target.value) : null,
              })
            }
            options={[
          ...
          <div className="flex gap-4">
            ...
            >
              <Banknote
                className={`w-5 h-5 ${saleType === "vista" ? "text-[#2A2933]" : "text-[#616167]"}`}
              />
              <span
                className={`font-semibold ${saleType === "vista" ? "text-[#2A2933]" : "text-[#616167]"}`}
              >
                À vista
              </span>
            </button>
            ...
            >
              <CreditCard
                className={`w-5 h-5 ${saleType === "prazo" ? "text-[#2A2933]" : "text-[#616167]"}`}
              />
              <span
                className={`font-semibold ${saleType === "prazo" ? "text-[#2A2933]" : "text-[#616167]"}`}
              >
                À prazo
              </span>
            </button>
          ...
          {saleType === "prazo" && (
            <div className="grid grid-cols-2 gap-4">
              <Input
                ...
                onChange={(e) =>
                  setFormData({ ...formData, first_due_date: e.target.value })
                }
              />
              <Input
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="text-[#2A2933] text-sm font-medium">
                Itens
              </label>
            </div>
            <div className="space-y-2">
              ...
              {formData.items.map((item, index) => (
                ...
                >
                  ...
                  />
                  <div>
                    <label className="text-[10px] text-[#939399] block mb-1">
                      Qtd
                    </label>
                    <Input
                      type="number"
                  ...
                  <div>
                    ...
                    <Input
                      value={
                        (item as { sale_price_display?: string })
                          .sale_price_display || ""
                      }
                      onChange={(e) =>
                        handleItemChange(
          ...
          {selectedInstallment && (
            <div className="p-4 bg-[#F8F6F4] rounded-xl">
              <p className="text-[#2A2933]">
                Parcela <strong>#{selectedInstallment.number}</strong> - Venda{" "}
                {selectedInstallment.sale}
              </p>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <p className="text-[#2A2933] font-medium">
                  {selectedSale.customer_name || "Sem cliente"}
                </p>
              </div>
              <div>
                <span className="text-[#939399] text-xs font-semibold">
              ...
              <div>
                ...
                <p className="text-[#2A2933] font-medium">
                  {selectedSale.installments_count}x
                </p>
              </div>
            </div>
            ...
            {selectedSale.installments.length > 0 && (
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  {selectedSale.installments.some(
                    (i) => i.status === "pending",
                  ) && (
                    <button
                      onClick={() => {
                    ...
                    >
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <span
                          ...
                          style={{
                            color:
                              inst.status === "paid" ? "#008A4E" : "#939399",
                            backgroundColor:
                              inst.status === "paid" ? "#008A4E20" : "#F8F6F4",
                          }}
                        >
